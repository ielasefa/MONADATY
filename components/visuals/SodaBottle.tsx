...
export function SodaBottle({ width = 220, height = 360, accent = "#D5B87D", label = "", className }: SodaBottleProps) {
  const uid = useId().replace(/:/g, "");
  const b1 = `b1-${uid}`;
  const fb = `bf-${uid}`;
  return (
    <svg className={className} width={width} height={height} viewBox="0 0 220 360" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label={label}>
      <defs>
