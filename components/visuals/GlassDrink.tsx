...
export function GlassDrink({ width = 260, height = 280, accent = "#D5B87D", label = "", className }: GlassDrinkProps) {
  const uid = useId().replace(/:/g, "");
  const gl1 = `g1-${uid}`;
  const fg = `gf-${uid}`;
  return (
    <svg className={className} width={width} height={height} viewBox="0 0 260 280" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label={label}>
      <defs>
