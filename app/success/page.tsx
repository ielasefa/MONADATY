...
<div className="relative mx-auto max-w-md space-y-8">
  <div className="mx-auto flex h-20 w-20 items-center justify-center rounded-md border border-burgundy/10 bg-burgundy/5 text-burgundy">
  ...
  <p className="text-[0.82rem] text-ivory/28">{t(translations, "order_not_found_desc", lang)}</p>
  ...
  return (
    <div className="container-premium py-16 md:py-24 lg:py-32">
      <div className="mx-auto max-w-3xl space-y-12 md:space-y-16">
          ...
          <section className="relative px-6 py-16 text-center md:px-12 md:py-20">
            <div className="relative mx-auto max-w-2xl space-y-8">
              ...
              <div className="space-y-4">
                ...
                <p className="mx-auto max-w-md text-[0.82rem] leading-[1.9] text-ivory/28">
                  {t(translations, "order_confirmed_desc", lang)}
                </p>
              </div>

              <div className="inline-flex items-center gap-3 rounded-md border border-ivory/[0.06] bg-ivory/[0.02] px-5 py-2.5">
                <span className="text-[0.55rem] uppercase tracking-[0.2em] text-ivory/28">{t(translations, "order_label", lang)}</span>
        ...
        <FadeIn delay={0.2}>
          <section className="space-y-6 text-center">
            {order.orderStatus === "delivered" || order.orderStatus === "completed" ? (
              <>
                <p className="text-[0.65rem] uppercase tracking-[0.2em] text-ivory/28">{t(translations, "delivered_on", lang)}</p>
              ...
              <>
                <p className="text-[0.65rem] uppercase tracking-[0.2em] text-ivory/28">{t(translations, "out_for_delivery", lang)}</p>
              ...
              <>
                <p className="text-[0.65rem] uppercase tracking-[0.2em] text-ivory/28">{t(translations, "estimated_delivery", lang)}</p>
                <p className="text-2xl font-medium text-ivory md:text-3xl">{estimatedDate}</p>
              </>
            )}
            <p className="text-[0.78rem] text-ivory/35">
              {order.orderStatus === "delivered" || order.orderStatus === "completed"
            ...
            {order.deliveryCompany && order.trackingNumber ? (
              <p className="mt-2 text-[0.75rem] text-ivory/50">
                {order.deliveryCompany}: {order.trackingNumber}
              </p>
        ...
        <FadeIn delay={0.3}>
          <section className="space-y-6">
            <p className="text-[0.48rem] uppercase tracking-[0.22em] text-ivory/20">{t(translations, "order_details", lang)}</p>
            <div className="h-px w-full bg-ivory/[0.04]" />
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "order_number", lang)}</p>
                <p className="font-mono text-[0.78rem] font-medium text-ivory">{order.orderNumber}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "date", lang)}</p>
                <p className="text-[0.78rem] font-medium text-ivory">{orderDate}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "payment_method", lang)}</p>
                <p className="text-[0.78rem] font-medium text-ivory">{paymentMethodLabel}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "delivery_status", lang)}</p>
                <p><span className="inline-flex rounded-md border border-ivory/[0.06] bg-ivory/[0.03] px-3 py-1 text-[0.52rem] font-semibold uppercase tracking-[0.2em] text-gold/70">{order.orderStatus}</span></p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "estimated_delivery", lang)}</p>
                <p className="text-[0.78rem] font-medium text-ivory">{estimatedDate}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "payment_status", lang)}</p>
                <p>
                  <span className={`inline-flex rounded-md px-3 py-1 text-[0.52rem] font-semibold uppercase tracking-[0.2em] ${order.paymentStatus === "paid" ? "border border-gold/15 bg-gold/5 text-gold/70" : "border border-ivory/[0.06] bg-ivory/[0.03] text-ivory/50"}`}>
                    {order.paymentStatus}
                  </span>
                </p>
              </div>
        ...
        <FadeIn delay={0.35}>
          <section className="space-y-6">
            <p className="text-[0.48rem] uppercase tracking-[0.22em] text-ivory/20">{t(translations, "customer_shipping", lang)}</p>
            <div className="h-px w-full bg-ivory/[0.04]" />
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "customer", lang)}</p>
                <p className="text-[0.78rem] font-medium text-ivory">{order.customerName}</p>
              </div>
              <div className="space-y-1.5">
                <p className="text-[0.55rem] text-ivory/20">{t(translations, "email", lang)}</p>
                <p className="text-[0.78rem] text-ivory/50">{order.customerEmail}</p>
              </div>
              {order.phone && (
                <div className="space-y-1.5">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "phone", lang)}</p>
                  <p className="text-[0.78rem] text-ivory/50">{order.phone}</p>
                </div>
              )}
              {order.address && (
                <div className="space-y-1.5 lg:col-span-2">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "shipping_address", lang)}</p>
                  <p className="text-[0.78rem] text-ivory/50">{order.address}</p>
                  {(order.city || order.postalCode) && (
                    <p className="text-[0.65rem] text-ivory/28">
              ...
              {order.deliveryCompany && (
                <div className="space-y-1.5">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "delivery_company", lang)}</p>
                  <p className="text-[0.78rem] text-ivory/50">{order.deliveryCompany}</p>
                </div>
              )}
              {order.trackingNumber && (
                <div className="space-y-1.5">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "tracking_number", lang)}</p>
                  <p className="font-mono text-[0.78rem] text-ivory/50">{order.trackingNumber}</p>
                </div>
              )}
              {actualDateDisplay && (
                <div className="space-y-1.5">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "actual_delivery", lang)}</p>
                  <p className="text-[0.78rem] text-ivory/50">{actualDateDisplay}</p>
                </div>
              )}
              {order.deliveryNotes && (
                <div className="space-y-1.5">
                  <p className="text-[0.55rem] text-ivory/20">{t(translations, "delivery_notes", lang)}</p>
                  <p className="text-[0.78rem] text-ivory/50">{order.deliveryNotes}</p>
                </div>
              )}
            </div>
          </section>
        ...
        <FadeIn delay={0.4}>
          <section className="space-y-6">
            <p className="text-[0.48rem] uppercase tracking-[0.22em] text-ivory/20">{t(translations, "order_summary", lang)}</p>
                ...
                <div key={`${item.productId}-${idx}`} className="flex items-center justify-between gap-5 py-5 first:pt-0 last:pb-0">
                  <div className="flex items-center gap-5 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-[0.6rem] text-ivory/28">{t(translations, "qty_label", lang).replace("{qty}", String(item.quantity)).replace("{price}", item.unitPrice)}</p>
                    </div>
                  </div>
            ...
            <div className="space-y-2.5 border-t border-ivory/[0.04] pt-5">
